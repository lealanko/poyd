...
(*testing for the synchronous functions*)
(*---------------------------------------------------------*)
let test_for_map_function () =
   let res = Comm.init Sys.argv Comm.Non_Fault_Tolerant Comm.Synch 
   and thearray = mk_array 15 5 in
   print_array !thearray;    
   Ftol.print_error ("so are you here?");
   
   let resarray = Parfunc.map square thearray in
   let length = Array.length !resarray in
   print_array !resarray;
   Comm.final ();;

(*scan function changes the order of the first two elements in the resulting
* array *)
let test_for_scan_function () =
   let res = Comm.init Sys.argv Comm.Non_Fault_Tolerant Comm.Synch 
   and thearray = mk_array 15 5 in
   print_array !thearray;    
   Ftol.print_error ("so are you here?");
